/**
DB Sistema Hotelero Alejandro (V1.1)
PostgreSQL version 17.10-2
Cambios v1.1: se agrega trigger para Historia de Usuario STK-01
**/
...
CREATE TABLE Movement_Stock_Detail(
    ...
    CONSTRAINT fk_detail_stock FOREIGN KEY (stock_id) REFERENCES Articles_Deposit_Stock(stock_id)
);
...
-- ============================================================
-- 9. HISTORIA STK-01: Registrar y administrar depósitos
-- ============================================================
-- Criterios de aceptación cubiertos acá:
--   - Nombre único de depósito -> ya cubierto por UNIQUE en deposit_name (sección 5)
--   - Estado activo/inactivo -> ya cubierto por deposit_state (sección 5)
--   - Un depósito inactivo no debe poder recibir nuevos movimientos de stock -> trigger de abajo
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE deposit_id = NEW.deposit_origin_id AND deposit_state = TRUE
    ) THEN
        RAISE EXCEPTION 'El depósito de origen (id=%) está inactivo y no puede recibir movimientos de stock', NEW.deposit_origin_id;
    END IF;

    IF NEW.deposit_destination_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM Deposit
        WHERE deposit_id = NEW.deposit_destination_id AND deposit_state = TRUE
    ) THEN
        RAISE EXCEPTION 'El depósito de destino (id=%) está inactivo y no puede recibir movimientos de stock', NEW.deposit_destination_id;
    END IF;
...
DROP TRIGGER IF EXISTS trg_check_deposit_active ON Stock_Movement;

CREATE TRIGGER trg_check_deposit_active
BEFORE INSERT ON Stock_Movement
FOR EACH ROW
EXECUTE FUNCTION fn_check_deposit_active();
