...
const service = require('./tiposComprobante.service');
...
const crear = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ ok: true, message: 'Tipo de comprobante creado.', data });
});

const actualizar = asyncHandler(async (req, res) => {
  const data = await service.actualizar(req.params.id, req.body);
  res.json({ ok: true, message: 'Tipo de comprobante actualizado.', data });
});

const cambiarEstado = asyncHandler(async (req, res) => {
  const data = await service.cambiarEstado(req.params.id, req.body?.active ?? req.body?.estado);
  res.json({
    ok: true,
