...
-------------------------------------------------------------------
-- 001_stk02_articulos.sql
-- Historia STK-02: Catálogo de Artículos

-- Categorías (si ya existe en tu BDD, este CREATE se ignora)
...
CREATE TABLE IF NOT EXISTS articles (
    ...
    article_name VARCHAR(150) NOT NULL,
    article_compound_name VARCHAR(200) NOT NULL,
    article_description VARCHAR(255),
    article_unit_of_measure VARCHAR(20) NOT NULL DEFAULT 'UNIDAD',
    article_stock_min_general NUMERIC(12,2) NOT NULL DEFAULT 0,
    article_state BOOLEAN NOT NULL DEFAULT TRUE,
    creation_date TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Si la tabla `articles` ya existía sin estos campos, esto la ajusta sin romper nada:
ALTER TABLE articles
    ADD COLUMN IF NOT EXISTS article_number VARCHAR(50) NOT NULL DEFAULT '',
    ADD COLUMN IF NOT EXISTS article_compound_name VARCHAR(200) NOT NULL DEFAULT '';

-- "El código de artículo no debe repetirse" -> ya cubierto por UNIQUE(article_code) arriba

CREATE INDEX IF NOT EXISTS idx_articles_category ON articles(category_id);

-- Nota sobre el 3er criterio de STK-02 ("debe quedar disponible para asociarse a cualquier
-- depósito"): articles no tiene ninguna FK hacia depósitos ni restricción que lo limite,
-- así que ya queda disponible por diseño. La asociación real (tabla de stock por depósito)
-- es responsabilidad de la historia STK-01, que maneja tu compañero.