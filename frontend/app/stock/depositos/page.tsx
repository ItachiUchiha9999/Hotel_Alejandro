...
export default function DepositosPage() {
  ...
  const depositosFiltrados = depositos.filter((deposito) => {
    ...
    return coincideNombre; // Si es "Estado: todos"
  });
  ...
  return (
    <div className="flex flex-col min-h-full w-full">
      ...
      {/* BARRA SUPERIOR CON FILTRADO ACTIVO */}
        ...
        <Card titulo="Depósitos registrados">
          {cargando ? (
            ...
            <div className="overflow-x-auto mt-2">
              <table className="w-full text-sm text-carbon">
                <thead className="border-b border-line bg-carbon/5 text-xs uppercase text-carbon/60">
                  <tr>
                    {/* 👇 NOMBRE A LA IZQUIERDA, ESTADO Y ACCIONES CENTRADOS */}
                ...
                <tbody>
                  {depositosFiltrados.map((deposito) => (
                    <tr key={deposito.id} className="border-b border-line hover:bg-carbon/5 transition-colors">
                      ...
                      <td className="px-4 py-3 font-medium text-left">{deposito.nombre}</td>
                      
                      {/* 👇 ESTADO CENTRADO */}
                      <td className="px-4 py-3 text-center">
                        <span className={`py-1 px-3 rounded-full text-[10px] font-semibold uppercase tracking-wider text-center w-28 inline-block ${
                      ...
                      <td className="px-4 py-3 text-center">
                        <Link 
                          ...
                          className="text-gold hover:text-gold-dark font-medium text-xs transition-colors inline-block"
                        >
                          Editar
                        </Link>
                      </td>
