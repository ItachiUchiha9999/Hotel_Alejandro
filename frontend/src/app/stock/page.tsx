...
export default function StockPage() {
    ...
    try {
      ...
      if (depositId === 'ALL') {
        ...
        const requests = activeDeposits.map((dep) =>
          fetch(`http://localhost:4000/api/stock/deposit/${dep.deposit_id}`)
  ...
  const handleConfirmarTransferencia = async () => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/stock/movements', {
        method: 'POST',
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-[#dedcd9] font-sans h-screen">
      {/* HEADER PRINCIPAL */}
