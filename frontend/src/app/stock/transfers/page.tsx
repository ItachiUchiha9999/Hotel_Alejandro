...
export default function TransfersPage() {
  ...
  // El backend todavía no expone GET /api/deposits (pendiente de STK-01).
  const loadAll = useCallback(async () => {
    ...
    await Promise.all(
      DEPOSIT_IDS.map(async (id) => {
        try {
          const res = await fetch(`${API}/api/stock/deposit/${id}`);
          if (!res.ok) return;
