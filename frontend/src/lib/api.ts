...
export function registrarMantenimiento(datos: DatosBloqueoMantenimiento) {
  return api.post<{ ok: boolean; message: string; data: any }>("/mantenimiento", datos as Record<string, unknown>);
}
...
export function cerrarMantenimiento(
  ...
  return api.patch<{ ok: boolean; message: string; data: any }>(`/mantenimiento/${id}/cerrar`, datos as Record<string, unknown>);
}

export function getMantenimientosActivos(roomId?: number) {
  const query = roomId ? `?roomId=${roomId}&abiertos=true` : "?abiertos=true";
  return api.get<any[]>(`/mantenimiento${query}`);
}
