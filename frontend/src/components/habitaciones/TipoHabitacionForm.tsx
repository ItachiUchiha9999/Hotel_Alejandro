...
export function TipoHabitacionForm({ tipo, onGuardado, onCancelar }: TipoHabitacionFormProps) {
  ...
  const [precio, setPrecio] = useState(String(tipo?.current_price ?? ""));
  ...
  const validar = (): Errores => {
    ...
    if (precio !== "" && (!Number.isFinite(Number(precio)) || Number(precio) <= 0)) {
      nuevos.precio = "Ingresá un precio mayor que cero.";
    }
    return nuevos;
  };
  ...
  const guardar = async (e: FormEvent) => {
    ...
    const datos = {
      ...
      base_price: precio === "" ? undefined : Number(precio),
    };
  ...
  return (
    <form onSubmit={guardar} noValidate className="flex flex-col gap-5">
      ...
      <Field
        ...
        ayuda="Se usará al registrar reservas para este tipo."
      >
        <Input
          id="tipo-precio"
          type="number"
