/**
 DB Sistema Hotelero Alejandro - versión consolidada STK-04 / STK-05
 PostgreSQL 17

 IMPORTANTE:
 - Se elimina la dependencia de type_operation_enum para Stock_Movement.
 - Los tipos de movimiento se administran mediante Movement_Type.
 - Movement_Stock_Detail referencia stock_id, no article_id.
**/
...
CREATE TABLE Movement_Stock_Detail(
    ...
    CONSTRAINT fk_detail_movement
        ...
        REFERENCES Stock_Movement(stock_movement_id)
        ON DELETE CASCADE,
    CONSTRAINT fk_detail_stock
        FOREIGN KEY (stock_id)
        REFERENCES Articles_Deposit_Stock(stock_id)
);
...
INSERT INTO Movement_Type (movement_type, description, effect, active)
VALUES
    ('INGRESO', 'Entrada de mercadería al stock', 'SUMA', TRUE),
    ('EGRESO', 'Salida de mercadería del stock', 'RESTA', TRUE),
    ('TRANSFERENCIA', 'Transferencia de mercadería entre depósitos', 'TRANSFERENCIA', TRUE);

-- Movimiento de ejemplo: INGRESO de Agua Mineral
INSERT INTO Stock_Movement (movement_type_id, deposit_origin_id, employees_id, supplier_id, observations)
VALUES (
    (SELECT movement_type_id FROM Movement_Type WHERE movement_type = 'INGRESO'),
    4,
    2,
    3,
    'Compra de reposición mensual de bebidas según Factura A-000123'
);

INSERT INTO Movement_Stock_Detail (stock_movement_id, stock_id, amount)
VALUES (
    1,
    (SELECT stock_id FROM Articles_Deposit_Stock WHERE article_id = 5 AND deposit_id = 4),
    120.00
);

-- Movimiento de ejemplo: TRANSFERENCIA de Toallones
INSERT INTO Stock_Movement (movement_type_id, deposit_origin_id, deposit_destination_id, employees_id, observations)
VALUES (
    (SELECT movement_type_id FROM Movement_Type WHERE movement_type = 'TRANSFERENCIA'),
    1,
    2,
    3,
    'Transferencia para stock de ropa blanca diaria'
);

INSERT INTO Movement_Stock_Detail (stock_movement_id, stock_id, amount)
VALUES (
    2,
    (SELECT stock_id FROM Articles_Deposit_Stock WHERE article_id = 2 AND deposit_id = 1),
    20.00
);
